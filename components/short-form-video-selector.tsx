...
type ShortFormVideoSelectorProps = {
  onUpdate: (selection: {
    plan: string
    planLabel: string
    price: number
...
export function ShortFormVideoSelector({ onUpdate }: ShortFormVideoSelectorProps) {
  const planOptions: PlanOption[] = [
    // Removed empty string option to prevent Select.Item error
    { label: "2 Videos per Month – $100/mo", value: "price_1RBAjRAog87WCP1E13c1426w", videos: 2, price: 100 },
    { label: "4 Videos per Month – $200/mo", value: "price_1RDxvkAog87WCP1ETmfheZd3", videos: 4, price: 200 },
    { label: "6 Videos per Month – $300/mo", value: "price_1RDxvlAog87WCP1EU9489Y9w", videos: 6, price: 300 },
    { label: "8 Videos per Month – $400/mo", value: "price_1RDxvlAog87WCP1EBN8NvOM4", videos: 8, price: 400 },
  ]

  const platformOptions: PlatformOption[] = [
    { icon: "Instagram", value: "price_1RDxvdAog87WCP1EZF2SzUI0", label: "Instagram" },
    { icon: "Tiktok", value: "price_1RDxveAog87WCP1EKU7ausS9", label: "TikTok" },
    { icon: "Facebook", value: "price_1RBAjRAog87WCP1EtPz0yB1p", label: "Facebook" },
    { icon: "Linkedin", value: "price_1RDxvfAog87WCP1Ecno5fc1n", label: "LinkedIn" },
    { icon: "Youtube", value: "price_1RBAjPAog87WCP1EE7zDAki7", label: "YouTube" },
    { icon: "Google", value: "price_1RDxvdAog87WCP1ElRAFBeNL", label: "Google" },
  ]

  const additionalPlatformOptions: PlatformOption[] = [
    { icon: "Instagram", value: "price_1RDxviAog87WCP1E37g1h0Da", label: "Instagram" },
    { icon: "Tiktok", value: "price_1RDxvjAog87WCP1EvvCKImdr", label: "TikTok" },
    { icon: "Facebook", value: "price_1RBAjOAog87WCP1Ennnv4MVK", label: "Facebook" },
    { icon: "Linkedin", value: "price_1RBAjNAog87WCP1EICrc7nxN", label: "LinkedIn" },
    { icon: "Youtube", value: "price_1RDxvjAog87WCP1EQAqzXMSp", label: "YouTube" },
    { icon: "Google", value: "price_1RBAjMAog87WCP1En5a3zZLq", label: "Google" },
  ]

  const [selectedPlan, setSelectedPlan] = useState<string>("price_1RBAjRAog87WCP1E13c1426w")
  const [freePlatform, setFreePlatform] = useState<string>("price_1RDxveAog87WCP1EKU7ausS9") // TikTok
  const [additionalPlatforms, setAdditionalPlatforms] = useState<string[]>([])

  const handlePlanChange = (value: string) => {
    if (!value) return;
    setSelectedPlan(value)
    const updatedPlan = planOptions.find((p) => p.value === value)
  ...
  return (
    <BaseServiceSelector
      ...
      subtitle="Engaging TikTok and Reels content that converts"
      icon={<Youtube /> as React.ReactElement}
      color="cyan"
    >
            ...
            {platformOptions.map((platform) => (
              <Button
                ...
                className={cn(
                  ...
                  freePlatform === platform.value && "border-blue-500 bg-zinc-700 text-white",
                )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {additionalPlatformOptions.map((platform) => (
              <div key={platform.value} className="flex items-center space-x-2">
                <Checkbox
                  id={`video-platform-${platform.value}`}
                ...
                />
                <Label
                  htmlFor={`video-platform-${platform.value}`}
                  className={cn(
                    "text-sm font-medium",
